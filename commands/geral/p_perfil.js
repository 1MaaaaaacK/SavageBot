...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        try {
            ...
            return message.channel.send('Erro no link');
        }
        ...
        let msg = await message.channel.send(PerfilInfoGenerating(message));
        ...
        try {
            PerfilInfos = {
                amigos: await fetch(
                    ...
                    .catch((error) => {
                        return msg.edit(PerfilWrong(message)).then((m) => m.delete({ timeout: 10000 }));
                    }),
                bans: await fetch(
            ...
            const embed = new Discord.MessageEmbed()
                .setColor('36393f')
                .setThumbnail(PerfilInfos.playerInfos[0].avatarfull)
                .setTitle(PerfilInfos.playerInfos[0].personaname)
                .addFields(
                    { name: 'Steamid', value: PerfilInfos.steamid, inline: true },
                    { name: '\u200B', value: '\u200B', inline: true },
                    { name: 'Vac', value: PerfilInfos.bans[0].VACBanned, inline: true },
                    { name: '\u200B', value: '\u200B' },
                    { name: 'TradeBan', value: PerfilInfos.bans[0].EconomyBan, inline: true },
                    { name: '\u200B', value: '\u200B', inline: true },
                    { name: 'CommunityBan', value: PerfilInfos.bans[0].CommunityBanned, inline: true },
                    { name: '\u200B', value: '\u200B' },
                    {
                        name: 'Total de Jogos',
                        value: PerfilInfos.jogos_totais,
                        inline: true,
                    },
                    { name: '\u200B', value: '\u200B', inline: true },
                    {
                        name: 'Level da Steam',
                        value: PerfilInfos.steam_lvl,
                        inline: true,
                    },
                    { name: '\u200B', value: '\u200B' },
                    { name: 'Total de Amigos', value: PerfilInfos.amigos, inline: true },
                    { name: '\u200B', value: '\u200B', inline: true },
                    {
